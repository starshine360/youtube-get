...
import youtube_get.utils.exceptions as exceptions

from youtube_get import __version__
from youtube_get import CaptionQuery, Playlist, Stream, YouTube
...
def main():
    ...
    if not args.url or "youtube" not in args.url:
        print(f"Received URL may be error, please check it -> {args.url}")
        sys.exit(1)

    if "/playlist" in args.url:
        print(f"Loading playlist from {args.url}")
        ...
        for youtube_video in playlist.videos:
            ...
            except exceptions.YouTubeError as err:
                print(f"There was an error with video: {youtube_video}")
                print(f"-> {repr(err)}")
    else:
        print(f"Loading video from {args.url}")
...
def _parse_args(
    ...
    parser.add_argument(
        "--resolution", type=str, help="The resolution for the desired stream",
    )
    parser.add_argument(
        "--list", action="store_true",
        help="The list option causes pytube cli to return a list of streams available to download"
    )
    parser.add_argument(
